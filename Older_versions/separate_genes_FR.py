...
if len(sys.argv) == 1:
    ...
    print "ex: python speratae_genes.py my_fastq_file.fastq forward"
    print ""
    print "If you want to run it for multiple files, use the shell:"
    print "for file in *_R1_001.fastq; do python separate_genes.py $file forward; done >> screen.out 2>> screen.err &"
    print ""
    sys.exit()
...
#Reverse primers:
p16S_r = '(GGACTAC\w\wGGGT\wTCTAAT){e<=1}' #R806
p18S_r = '(TACAAAGGGCAGGGACGTAAT){e<=1}' # B18Sr2B 
pUPA_r = '(TCAGCCTGTTATCCCTAGAG){e<=1}' #p23SrV_r1 
ptufA_r = '(CCTTC\wCGAAT\wGCRAA\wCGC){e<=1}' #tufAR
